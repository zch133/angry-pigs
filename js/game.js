...
const PIG_TYPES = {
  normal: { name: '普通猪', emoji: '🐷', colorHex: 0xFFB6C1 },
  speed:  { name: '飞速猪', emoji: '💨', colorHex: 0xFFE066 },
  bomb:   { name: '炸弹猪', emoji: '💣', colorHex: 0x444444 },
};
...
function initGame() {
  ...
  // 橡皮筋: 从弹弓两叉口到猪位置, 拖拽时跟随
  G.bandL = new THREE.Mesh(
    new THREE.CylinderGeometry(0.02, 0.02, 1, 6),
    new THREE.MeshPhongMaterial({ color: 0x3B2110 })
  );
  G.bandR = new THREE.Mesh(
    new THREE.CylinderGeometry(0.02, 0.02, 1, 6),
    new THREE.MeshPhongMaterial({ color: 0x3B2110 })
  );
  G.bandL.visible = false; G.bandR.visible = false;
...
function setupInput() {
  ...
  const onUp = e => {
    ...
    // 弹弓回弹动画
    if (G.slingshot) {
      G.slingshot.rotation.z = 0.3;
      setTimeout(() => { if (G.slingshot) G.slingshot.rotation.z = 0; }, 100);
    }
    if (G.pullDist < 0.3) {
      G.pigMesh.position.set(SLINGSHOT_POS.x, SLINGSHOT_POS.y, SLINGSHOT_POS.z);
      G.pullDist = 0;
...
function updateSlingshotBands() {
  if (!G.pigMesh || !G.bandL) return;
  const forkL = new THREE.Vector3(SLINGSHOT_POS.x - 0.15, SLINGSHOT_POS.y + 0.2, 0);
  const forkR = new THREE.Vector3(SLINGSHOT_POS.x + 0.15, SLINGSHOT_POS.y + 0.2, 0);
  const pig = G.pigMesh.position;
  updateBandMesh(G.bandL, forkL, pig);
  updateBandMesh(G.bandR, forkR, pig);
  G.bandL.visible = true; G.bandR.visible = true;
}
function updateBandMesh(band, from, to) {
...
function checkCollisions() {
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    if (block) {
      ...
      // squash & stretch: 撞击瞬间猪压扁
      if (pig.mesh) {
        pig.mesh.scale.set(1.3, 0.7, 1.3);
        setTimeout(() => { if (pig.mesh) pig.mesh.scale.set(1, 1, 1); }, 100);
      }
...
function destroyBlock(block) {
  ...
  // 碎片: 按材质不同的数量/大小/颜色
  const debrisCount = { ice: 8, wood: 5, stone: 4 }[block.type];
  const debrisSize = { ice: 0.08, wood: 0.12, stone: 0.15 }[block.type];
  for (let i = 0; i < debrisCount; i++) {
    const d = Models.createDebris(MAT_COLOR[block.type], debrisSize + Math.random() * 0.06);
    d.position.copy(block.mesh.position);
    d.position.x += (Math.random() - 0.5) * block.w;
    d.position.y += (Math.random() - 0.5) * block.h;
    d.position.z += (Math.random() - 0.5) * block.d;
    // 给碎片物理速度
    d.userData.vx = (Math.random() - 0.5) * 4;
    d.userData.vy = Math.random() * 3 + 1;
    d.userData.vz = (Math.random() - 0.5) * 4;
    d.userData.vrot = (Math.random() - 0.5) * 0.3;
    d.material.transparent = block.type === 'ice';
    d.material.opacity = block.type === 'ice' ? 0.7 : 1.0;
    G.scene.add(d);
    G.debrisPieces = G.debrisPieces || [];
    G.debrisPieces.push(d);
  }
  
  // 冰碎裂额外冰碴 (小白色粒子)
  if (block.type === 'ice') {
    for (let i = 0; i < 6; i++) {
      const spark = Models.createDebris(0xFFFFFF, 0.04);
      spark.position.copy(block.mesh.position);
      spark.position.x += (Math.random() - 0.5) * block.w;
      spark.position.y += (Math.random() - 0.5) * block.h;
      spark.position.z += (Math.random() - 0.5) * block.d;
      spark.userData.vx = (Math.random() - 0.5) * 6;
      spark.userData.vy = Math.random() * 4 + 2;
      spark.userData.vz = (Math.random() - 0.5) * 6;
      spark.userData.vrot = 0;
      spark.material.transparent = true;
      spark.material.opacity = 0.8;
      G.scene.add(spark);
      G.debrisPieces.push(spark);
    }
  }
  ...
  if (block.type === 'stone') {
    for (let i = 0; i < 4; i++) {
      const dust = Models.createDebris(0xAAAAAA, 0.2);
      dust.position.copy(block.mesh.position);
      dust.position.x += (Math.random() - 0.5) * block.w;
      dust.position.y += (Math.random() - 0.5) * block.h;
      dust.position.z += (Math.random() - 0.5) * block.d;
      dust.userData.vx = (Math.random() - 0.5) * 1;
      dust.userData.vy = Math.random() * 1;
      dust.userData.vz = (Math.random() - 0.5) * 1;
      dust.userData.vrot = 0;
      dust.material.transparent = true;
      dust.material.opacity = 0.3;
      G.scene.add(dust);
      G.debrisPieces.push(dust);
    }
  }
...
function updateFlyingPigs(dt) {
  G.flyingPigs.forEach(pig => {
    ...
    // 飞行拖尾: 每隔几帧生成一个小球, 逐渐缩小消失
    if (speed > 3 && !pig.stopped) {
      pig.trailCounter = (pig.trailCounter || 0) + 1;
      if (pig.trailCounter % 2 === 0) {
        const dot = new THREE.Mesh(
          new THREE.SphereGeometry(0.15, 6, 4),
          new THREE.MeshBasicMaterial({ color: PIG_TYPES[G.pigQueue[pig.pigIdx]?.type || 'normal'].colorHex || 0xFFB6C1, transparent: true, opacity: 0.5 })
        );
        dot.position.copy(p);
        dot.userData.life = 300;
        dot.userData.maxLife = 300;
        G.scene.add(dot);
        G.trailDots = G.trailDots || [];
        G.trailDots.push(dot);
      }
    }
...
function updateCamera(dt) {
  if (G.cameraMode === 'follow' && G.cameraFollowTarget) {
    ...
    const v = G.cameraFollowTarget.velocity;
    G.cameraTarget = { x: p.x, y: p.y, z: p.z };
    // 前瞻: 相机看向猪速度方向前方一点
    const lookAhead = 2;
    const lookX = p.x + v.x * lookAhead * 0.1;
    const lookY = p.y + v.y * lookAhead * 0.1;
    const lookZ = p.z + v.z * lookAhead * 0.1;
    const targetPos = new THREE.Vector3(p.x + 6, p.y + 3, p.z + 6);
    G.camera.position.lerp(targetPos, 0.12); // 从0.08提到0.12, 更快跟上
...
function animate(time) {
  ...
  // 拖尾粒子更新
  if (G.trailDots && G.trailDots.length > 0) {
    for (let i = G.trailDots.length - 1; i >= 0; i--) {
      const d = G.trailDots[i];
      d.userData.life -= dt;
      const t = d.userData.life / d.userData.maxLife;
      d.scale.setScalar(Math.max(0.01, t));
      d.material.opacity = t * 0.5;
      if (d.userData.life <= 0) { G.scene.remove(d); G.trailDots.splice(i, 1); }
    }
  }

  // 碎片物理更新
  if (G.debrisPieces && G.debrisPieces.length > 0) {
    const ddt = dt * 0.001;
    for (let i = G.debrisPieces.length - 1; i >= 0; i--) {
      const d = G.debrisPieces[i];
      d.userData.vy += GRAVITY * ddt;
      d.position.x += d.userData.vx * ddt;
      d.position.y += d.userData.vy * ddt;
      d.position.z += d.userData.vz * ddt;
      d.rotation.x += d.userData.vrot;
      d.rotation.z += d.userData.vrot * 0.7;
      if (d.position.y < 0) {
        d.position.y = 0;
        d.userData.vy *= -0.3;
        d.userData.vx *= 0.5;
      ...
      // 2秒后消失
      d.userData.life = (d.userData.life || 2000) - dt;
      if (d.userData.life <= 0) {
        G.scene.remove(d);
        G.debrisPieces.splice(i, 1);
      }
    }
  }
